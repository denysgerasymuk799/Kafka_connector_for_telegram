...
async def download_dialog(client, id, MSG_LIMIT, tg_producer, topic):
    """
    ...
    for m in messages:
        ...
        dialog.append(
            {
                "id": m.id,
                "from_id": str(m.from_id),
                "to_id": str(msg_attrs["to_id"]),
                "fwd_from": str(m.fwd_from),
    ...
    tg_producer.send_messages(topic, dialog)
...
if __name__ == "__main__":
    ...
    CONFIG_PATH = 'config_collect_data.json'
    MSG_LIMIT = 500
    ...
    # DIALOGS_ID = dialogs_id_input_handler(DIALOGS_IDS, dialogs_list)
